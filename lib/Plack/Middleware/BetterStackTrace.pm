...
sub internal_call {
    ...
    if ($opt{method} eq 'eval') {
        ...
        my $prefix = '(); package ' . $frame->package;
        my @ret = eval {
...
sub render_html {
    ...
    my $tx = Text::Xslate->new(
        ...
        function => {
            ...
            context_html => Text::Xslate::html_builder(\&context_html),
        }
    );
    ...
    <section class="backtrace">
        ...
        [% FOREACH frame IN backtrace_frames %]
            <div class="frame_info" id="frame_info_[% loop.index %]" style="display:none;">
                [% INCLUDE "variables_info" WITH frame = frame.frame, args = frame.args %]
            </div>
        [% END %]
    </section>
</body>
...
(function() {
    ...
    function apiCall(method, opts, cb) {
        ...
        req.open("POST", "[% script_name %]/__better_errors/" + OID + "/" + method, true);
...
<div class="sub">
    ...
    <div class='inset variables'>
        <table class="var_table">
            <tr><td class="name">Request parameters</td><td><pre>[% request.parameters.as_hashref_mixed | dump %]</pre></td></tr>
            [% IF request.session %]
            <tr><td class="name">Session</td><td><pre>[% request.session | dump %]</pre></td></tr>
            [% END %]
        </table>
    </div>
</div>
